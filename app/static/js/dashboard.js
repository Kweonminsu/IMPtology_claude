document.addEventListener('DOMContentLoaded', function() {
  ...
  if (refreshButton) {
    refreshButton.addEventListener('click', function() {
      ...
      const timeRange = activeTimeButton ? activeTimeButton.textContent.trim() : '오늘';
...
function updateMetricCards(timeRange) {
  ...
  const metrics = {
    ...
    '이번 달': { dataSources: 51, dataTables: 189, insights: 1248, reports: 95 },
...
function updateMetricValue(metricClass, value) {
  ...
  if (metricElement) {
    const valueElement = metricElement.closest('.metric-card').querySelector('.metric-value');
    if (valueElement) {
      // 숫자 애니메이션 효과로 값 업데이트
      animateNumber(valueElement, parseInt(valueElement.textContent.replace(/,/g, '')), value);
    }
  }
}
...
function initRelationshipDiagram() {
  // 중앙 노드 애니메이션
  const centralNode = document.querySelector('.relationship-node.central');
  if (centralNode) {
    // 맥박 효과는 CSS에서 처리됨

    // 연결선 애니메이션 (실제 구현시 SVG나 Canvas를 사용하여 더 복잡한 애니메이션 가능)
    const connectionLines = document.querySelectorAll('.connection-line');
    connectionLines.forEach(line => {
      line.style.animation = 'pulseLine 3s infinite';
    });
  }
}
