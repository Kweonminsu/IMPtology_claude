...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveNotice(event) {
        ...
        if (currentNoticeId) {
            // 기존 공지사항 수정
            const noticeIndex = sampleNotices.findIndex(notice => notice.id === currentNoticeId);
            if (noticeIndex !== -1) {
                ...
                // 원본 데이터도 업데이트
                const originalIndex = originalNotices.findIndex(notice => notice.id === currentNoticeId);
                if (originalIndex !== -1) {
            ...
            // 새 공지사항 추가
            const newId = sampleNotices.length > 0 ? Math.max(...sampleNotices.map(notice => notice.id)) + 1 : 1;
    ...
    function deleteNotice(id) {
        ...
        // 원본 배열에서도 제거
        const originalIndex = originalNotices.findIndex(notice => notice.id === id);
        if (originalIndex !== -1) {
