...
function sortReports(sortType) {
  ...
  const reportCards = Array.from(
    document.querySelectorAll(".report-card:not(.new-report)")
  );
  const newReportCard = document.querySelector(".new-report");
  ...
  reportCards.sort((a, b) => {
    if (sortType === "최신순") {
      ...
      const viewsA = parseInt(
        a.querySelector(".report-views").textContent.replace("회 조회", "")
      );
      const viewsB = parseInt(
        b.querySelector(".report-views").textContent.replace("회 조회", "")
      );
...
function openShareDialog(reportTitle) {
  ...
  modal.querySelector(".share-button").addEventListener("click", () => {
    document.body.removeChild(modal);
    showNotification(
      `"${reportTitle}" 리포트가 성공적으로 공유되었습니다.`,
      "success"
    );
  });
...
function useTemplate(templateTitle) {
  ...
  showNotification(
    `"${templateTitle}" 템플릿을 사용하여 새 리포트를 생성합니다.`,
    "info"
  );
