...
async def read_report(
    ...
    if not report.is_public and report.owner_id != current_user.id and not current_user.is_superuser:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="이 리포트에 접근할 권한이 없습니다")
...
async def schedule_report(
    ...
    return create_scheduled_report(db, report_id, frequency, recipients, current_user.id)
