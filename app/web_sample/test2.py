...
TABLES_METADATA = {
    ...
    "B": {
        ...
        "joinable_columns": {"A": ["id"], "C": ["line_number"]},
    },
    "C": {
        "name": "설비 상태",
        ...
        "joinable_columns": {"B": ["line_number"]},
    },
    "D": {
        "name": "원자재 정보",
...
def build_sql_query(request: AnalysisRequest) -> str:
    ...
    join_clause = []
    where_clause = [
        f"date BETWEEN '{request.dateRange.start}' AND '{request.dateRange.end}'"
    ]
    ...
    if len(selected_tables) > 1:
        if not request.joinConditions:
            raise HTTPException(
                status_code=400, detail="Join conditions required for multiple tables"
            )
    ...
    for filter_info in request.filters:
        ...
        if op == "LIKE":
            where_clause.append(
                f"{t_alias}.{filter_info.column} {op} '%{filter_info.value}%'"
            )
        elif op == "IN":
            values = filter_info.value.split(",")
            formatted_values = ", ".join([f"'{v.strip()}'" for v in values])
            where_clause.append(
                f"{t_alias}.{filter_info.column} {op} ({formatted_values})"
            )
        else:
...
def process_analysis(df: pd.DataFrame, request: AnalysisRequest) -> Dict[str, Any]:
    ...
    if request.analysisType == "summary":
        ...
        raise HTTPException(
            status_code=400, detail=f"Unsupported analysis type: {request.analysisType}"
        )


def process_summary_analysis(
    ...
    return {"type": "summary", "columns": columns, "rows": rows}
...
def process_trend_analysis(
    ...
    if not request.targetColumn:
        raise HTTPException(
            status_code=400, detail="Target column required for trend analysis"
        )
    ...
    if target_col_name not in df.columns:
        raise HTTPException(
            status_code=400, detail=f"Target column {target_col_name} not found"
        )
    ...
    if not date_col:
        raise HTTPException(
            status_code=400, detail="Date column required for trend analysis"
        )
    ...
    if not pd.api.types.is_datetime64_dtype(df[date_col]):
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Unable to convert {date_col} to date format"
            )
    ...
    if request.groupBy:
        ...
        if group_col_name == date_col:
            ...
            df[date_col] = df[date_col].dt.date
            grouped = (
                df.groupby([date_col, group_col_name])[target_col_name]
                .mean()
                .reset_index()
            )
        ...
        for _, row in df.iterrows():
            rows.append(
                [
                    row[date_col].strftime("%Y-%m-%d"),
                    (
                        float(row[target_col_name])
                        if pd.api.types.is_numeric_dtype(df[target_col_name])
                        else str(row[target_col_name])
                    ),
                ]
            )

    return {"type": "trend", "columns": columns, "rows": rows}
...
def process_correlation_analysis(
    ...
    if numeric_df.shape[1] < 2:
        raise HTTPException(
            status_code=400,
            detail="At least two numeric columns required for correlation analysis",
        )
    ...
    for i, col1 in enumerate(corr_matrix.columns):
        for j, col2 in enumerate(corr_matrix.columns):
            if i < j:  # Upper triangle only
                ...
                rows.append([display_col1, display_col2, float(corr_value.round(2))])

    # Sort by absolute correlation value (descending)
    rows.sort(key=lambda x: abs(x[2]), reverse=True)

    return {"type": "correlation", "columns": columns, "rows": rows}
